...
class GasGithubPanel extends HTMLElement {
  ...
  _renderShell_() {
    DomUtils.setHTML(this.shadowRoot, `
      <style>
        ...
        :host::before { border-bottom: 8px solid var(--gc-border); }
        :host::after  { top: -7px; border-bottom: 8px solid var(--gc-bg-elevated); }
        @keyframes qc__gh-in {
        ...
        .qc__gh-colLeft::-webkit-scrollbar { width: 8px; }
        .qc__gh-colLeft::-webkit-scrollbar-track { background: transparent; }
        .qc__gh-colLeft::-webkit-scrollbar-thumb {
        ...
        .qc__gh-ddItem:hover { background: var(--gc-accent-dim); }
        .qc__gh-ddItem.qc__gh-active { background: var(--gc-accent-dim); }
        ...
        .qc__gh-ddFooterBtn:hover { background: var(--gc-accent-dim); }
        ...
        .qc__gh-diff2html .d2h-info .d2h-code-line-prefix,
        .qc__gh-diff2html .d2h-info .d2h-code-line-ctn {
          color: var(--gc-text-muted) !important;
        }
        ...
        :host {
          --qc-gh-hl-string:   #a5d6ff;
          --qc-gh-hl-builtin:  #d2a8ff;
          --qc-gh-hl-tag:      #7ee787;
        }
        :host([theme="light"]) {
          --qc-gh-hl-string:   #0a3069;
          --qc-gh-hl-builtin:  #8250df;
          --qc-gh-hl-tag:      #116329;
        }
        ...
        .qc__gh-diff2html .hljs-link { color: var(--qc-gh-hl-string) !important; }
        ...
        .qc__gh-diff2html .hljs-built_in { color: var(--qc-gh-hl-builtin) !important; }
        ...
        .qc__gh-connectCard {
          padding: 14px;
          border: 1px dashed var(--gc-accent);
          border-radius: 10px;
          background: var(--gc-accent-dim);
          text-align: center;
        }
        ...
        .qc__gh-codeBox {
          ...
          background: var(--gc-bg-elevated);
          border: 1px dashed var(--gc-accent); border-radius: 8px;
        }
        .qc__gh-Code {
          flex: 1;
          font-family: "Roboto Mono", Consolas, monospace;
        ...
        .qc__gh-progress {
          ...
          color: var(--gc-text);
          overflow: hidden;
          animation: qc__gh-progressIn .18s ease-out;
        }
        @keyframes qc__gh-progressIn {
        ...
        .qc__gh-progressHead:hover { background: var(--gc-hover-soft); }
        .qc__gh-progressIcon {
        ...
        .qc__gh-progressActions {
          display: flex; align-items: center; gap: 2px;
          flex: 0 0 auto;
        }
        .qc__gh-progressChevron {
          font-size: 18px !important;
          color: var(--gc-text-muted);
          transition: transform .15s;
        }
        .qc__gh-progress.qc__gh-progressOpen .qc__gh-progressChevron {
        ...
        .qc__gh-progress.qc__gh-progressOpen .qc__gh-progressBody {
          display: block;
        }
        .qc__gh-progressPhase {
          display: flex; align-items: flex-start; gap: 8px;
          padding: 6px 10px;
          border-radius: 6px;
        }
        .qc__gh-progressPhase + .qc__gh-progressPhase {
          margin-top: 2px;
        }
        .qc__gh-progressPhaseIcon {
          width: 18px; height: 18px;
          display: grid; place-items: center;
          flex: 0 0 auto;
          margin-top: 1px;
        }
        .qc__gh-progressPhaseIcon .material-icons { font-size: 16px; }
        .qc__gh-progressPhase.pending .qc__gh-progressPhaseIcon .material-icons {
          color: var(--gc-text-faint);
        }
        .qc__gh-progressPhase.active .qc__gh-progressPhaseIcon .material-icons {
          color: var(--gc-accent);
        }
        .qc__gh-progressPhase.done .qc__gh-progressPhaseIcon .material-icons {
          color: var(--gc-green-text);
        }
        .qc__gh-progressPhase.error .qc__gh-progressPhaseIcon .material-icons {
          color: var(--gc-red-text);
        }
        .qc__gh-progressPhaseText {
          flex: 1; min-width: 0;
          line-height: 1.35;
        }
        .qc__gh-progressPhaseLabel {
          font-weight: 500;
          color: var(--gc-text);
        }
        .qc__gh-progress .qc__gh-progressPhase.pending .qc__gh-progressPhaseLabel,
        .qc__gh-progress .qc__gh-progressPhase.pending .qc__gh-progressPhaseHint {
          color: var(--gc-text-faint);
        }
        .qc__gh-progressPhaseHint {
          color: var(--gc-text-muted);
          font-size: 11.5px;
        }
        .qc__gh-progressFiles {
          margin-top: 4px;
          padding-left: 0;
          list-style: none;
      ...
      <div class="qc__gh-shell">
        ...
        <div id="ghProgress" class="qc__gh-progress" style="display:none"></div>
      </div>
    `);
  }
  ...
  async _handleAuthError_(response) {
    ...
    if (!is401 && !is403WithAuthMessage) return false;
  ...
  _renderDiffInto_(mount, item) {
    ...
    if (window.Diff?.createPatch && window.Diff2HtmlUI) {
      try {
        ...
        // `context: 3` → mismas líneas de contexto que GitHub muestra
        // por defecto en sus PR diffs. jsdiff usa 4 si no se pasa, lo
        // que metía una línea extra arriba/abajo de cada hunk y hacía
        // que diffs pequeños parecieran un archivo entero.
        const patch = window.Diff.createPatch(
          item.path, oldText, newText, 'remote', 'local',
          { context: 3 },
        );
    ...
    if (remote.size === 0 && local.size > 0) {
      console.warn(
        `[gas-github-panel] Remote returned 0 files but local has ${local.size}.`,
        {
          repo:       this._project.repo,
          branch:     this._project.branch,
          basePath:   this._project.basePath || '(none)',
          rawCount:   (res.data?.files || []).length,
          firstLocal: [...local.keys()].slice(0, 5),
        },
      );
    }
  ...
  async _pull_() {
    ...
    // Lista de archivos que efectivamente se van a aplicar al pull
    // (excluye los 'add' locales que no se tocan).
    const incoming = items.filter((i) => i.status !== 'add');
    this._openProgress_({
      kind:  'pull',
      title: `Pulling from ${this._project.repo}@${this._project.branch}`,
      files: incoming,
    });
    // Las dos primeras fases ya las hizo el FETCH_FILES previo (que
  ...
  async _pullWithApi_(items) {
    ...
    // Ctrl+S preventivo: si lo hicieran, sobrescribirían el contenido
    // recién aplicado con el código viejo del editor y revertirían el
    // pull, devolviendo la lista de cambios a "pendientes" al reabrir
    // el panel.
    this._monacoStale = true;
   ...
   */
  _updateAnchorBadge_() {
    const containerGithub_ = document.querySelector('#buttonGithubGas .yggLIc');
    ...
    // Asignamos el titulo adecuado
    containerGithub_.setAttribute('data-tt', tooltip);

    // Recorremos cada uno de los botones con el mismo ID por si Google recrea los paneles
    buttons.forEach((btn) => {
      const wrapper = btn.closest('.RO63ad');
      if (wrapper) wrapper.setAttribute('data-tt', tooltip);

      let dot = btn.querySelector('.qc__gh-dot');
      // El anillo del badge replica el color de fondo del botón
      // contenedor para que parezca "recortado". En dark el fondo es
      // ~#1e2027 y un anillo blanco se ve como un punto luminoso fuera
      // de tono; en light el fondo del botón es claro y necesita un
      // borde claro para no parecer una sombra suelta.
      const isDark = document.body.classList.contains('gc__is-dark-mode');
      const ringColor = isDark ? '#1e2027' : '#fff';
      if (isConnected && !dot) {
        ...
        dot.style.cssText = [
          ...
          'background:#34a853',`box-shadow:0 0 0 2px ${ringColor}`,
          'pointer-events:none','z-index:1',
        ].join(';');
        if (getComputedStyle(btn).position === 'static') btn.style.position = 'relative';
        btn.appendChild(dot);
      } else if (isConnected && dot) {
        // Repintamos el ring por si cambió el tema desde la última
        // creación del dot (toggle de "IDE Dark Mode").
        dot.style.boxShadow = `0 0 0 2px ${ringColor}`;
      } else if (!isConnected && dot) {
        dot.remove();
      }
    });
  ...
  // ─────────────────────────────────────────────
  // PROGRESS PANEL (push/pull)
  // Sustituye el toast "Sending changes…" por un mini-bloque colapsable
  // anclado al pie del panel. Permite al usuario ver qué fase está en
  // curso y la lista de archivos involucrados sin tapar el resto de
  // la UI ni forzar a esperar.
  // ─────────────────────────────────────────────

  /**
   * Configura las fases canónicas para una operación. Cada fase tiene:
   *   - id: clave única para `_setProgressPhase_`.
   *   - label: texto principal.
   *   - icon: material icon mostrado al lado.
   *   - hint?: descripción complementaria (opcional).
   *   - files?: lista de paths involucrados (opcional, expandida en el body).
   *
   * @param {'push'|'pull'} kind
   * @param {Array<{path:string}>} files
   * @returns {Array<{id:string,label:string,icon:string,files?:Array<string>}>}
   * @private
   */
  _buildProgressPhases_(kind, files) {
    const filePaths = (files || []).map((f) => f.path);
    if (kind === 'push') {
      return [
        { id: 'snapshot', label: 'Reading project snapshot',
          icon: 'cloud_download',
          hint: 'Asking Apps Script API for the current state.' },
        { id: 'blobs', label: 'Uploading file blobs',
          icon: 'upload_file',
          hint: `${filePaths.length} file(s) queued.`,
          files: filePaths },
        { id: 'tree', label: 'Building Git tree',
          icon: 'account_tree' },
        { id: 'commit', label: 'Creating commit',
          icon: 'commit' },
        { id: 'ref', label: 'Updating branch ref',
          icon: 'alt_route' },
      ];
    }
    // pull
    return [
      { id: 'tree', label: 'Reading remote tree',
        icon: 'account_tree' },
   ...
   * @private
   */
  _openProgress_({ kind, title, files }) {
  ...
  /**
   * Marca una fase como `active`/`done`/`error` y actualiza la UI. Si
   * la fase recibe `meta` (ej. "3/9 files"), la añade junto al título
   * principal del header.
   *
   * @param {string} phaseId
   * @param {'active'|'done'|'error'} status
   * @param {{meta?:string, hint?:string, files?:Array<string>}} [opts]
   * @private
   */
  _setProgressPhase_(phaseId, status, opts = {}) {
    const s = this._progressState;
    if (!s) return;
    const phase = s.phases.find((p) => p.id === phaseId);
    if (!phase) return;
  ...
  /**
   * Cierra el panel de progreso al instante.
   * @private
   */
  _closeProgress_() {
    const root = this.shadowRoot.getElementById('ghProgress');
  ...
  /**
   * Marca la operación como completada (éxito o fallo). Mantiene el
   * panel visible para que el usuario pueda revisar el listado, pero
   * cambia el icono del header y revela el botón de cerrar.
   *
   * @param {{success:boolean, title?:string, meta?:string}} opts
   * @private
   */
  _finishProgress_({ success, title, meta }) {
    const s = this._progressState;
    if (!s) return;
    s.finished = true;
    s.success = !!success;
